...
////////////////////////////////////////
    ...
    /**
     * Class to apply to fields which failed validation
     * @var string
     */
    public static $error_class = '';
    /**
     * Class to apply to fields which pass validation
     * @var string
     */
    public static $valid_class = '';
    /**
     * Path to icon to display error on validation checking
     * @var string
     */
    public static $validation_error_icon = '/images/silk/error.png';
    /**
     * Display size of validation error icon
     * @var integer
     */
    public static $validation_error_icon_size = 16;
    /**
     * The preface for the list of error messages
     * @var string
     */
    public static $has_errors_preface = 'There were errors with your submission:';
    /**
     * The glue to bind together each error message
     * @var string
     */
    public static $has_errors_glue = '<br />&bull; ';
    /**
...
////////////////////////////////////////
    ...
    {
        if (self::hasErrors()) {
            return self::$has_errors_preface.self::$has_errors_glue.implode(self::$has_errors_glue, self::$validation);
        }
        return '';
     ...
     *
     * @deprecated Never really implemented
     *
     ...
     *
     * @deprecated Deprecated in favor of using native DB handler parameter binding.
     *
    ...
    public static function getErrors($fields='')
    {
        ...
        $return = '';
        if ($fields=='') {
            $return = implode(self::$has_errors_glue, self::$validation);
        } else {
            $fields = self::str2Array($fields);
            foreach ($fields as $field) {
                if (!empty(self::$validation[$field])) {
                    $return .= (!empty($return) ? self::$has_errors_glue : '').self::$validation[$field];
                }
            }
        }
    ...
    /**
     * Set an array field(s) with the values of the array. The array must
     * have keys named with the $value_key and $id_key or nothing will be
     * loaded.
     *
     * This is primarily for preloading an array field with values from a recordset.
     *
     * @param string|array $field_names  List of field names to set their values
     * @param array        $array        Associative array of data
     * @param string       $value_key    Key name for the $array value field
     * @param string       $id_key       Key name for the $array id field (Default: 'id')
     * @version 2014-02-28.00
     */
    public static function setArrayField($field_names, array $array, $value_key, $id_key='id')
    {
    ...
    public static function field($field_name, $size=0, $max_size=255, $options=array())
    {
        ...
        if ($size > 0) {
            if ($options['with_validation']) {
                $return .= self::checkValidation($field_name);
            }
            $return .= '<input';
            ...
            if ($options['tabindex']!='') {
                $return .= ' tabindex="'.$options['tabindex'].'"';
            }
            ...
            if ($max_size>0 && $max_size<255) {
                $return .= ' maxlength="'.$max_size.'"';
            }
            if ($options['rel']!='') {
                $return .= ' rel="'.$options['rel'].'"';
            }
            if ($options['style']!='') {
                $return .= ' style="'.$options['style'].'"';
            }
            if ($options['extra_attributes']!='') {
                $return .= ' '.$options['extra_attributes'];
            }
            ...
            if ($options['with_class']!='') {
                $classes[] = $options['with_class'];
            }
            if ($options['with_validation'] && !self::isValid($field_name) && self::$error_class!='') {
                $classes[] = self::$error_class;
            } elseif ($options['with_validation'] && !$options['without_valid_class'] && isset(self::$form[$field_name]) && self::$form[$field_name]!='' && isset($_POST['Submit']) && self::$valid_class!='') {
                $classes[] = self::$valid_class;
            }
            if (count($classes)>0) {
    ...
    public static function radio($field_name, $value, $label='', $options=array())
    {
        ...
        if ($label=='') {
            $label=$value; // if no label, use value as the label
        }
        $return = '';
        if ($options['with_validation']) {
            $return .= self::checkValidation($field_name); // validate if asked
        }
        $return .= '<input';
        $return .= ' type="radio"';
        $return .= ' name="'.htmlspecialchars($field_name).'"';
        ...
        $return .= ' id="'.$id.'"';
        $return .= ' value="'.($options['filter_value'] ? htmlspecialchars($value) : $value).'"';
        if (isset(self::$form[$field_name]) && self::$form[$field_name]==$value) {
            $return .= ' checked'; // check the radio if it should be set
        }
        if ($options['required']) {
            $return .= ' required';
        }
        if ($options['input_class']!='') {
            $return .= ' class="'.$options['input_class'].'"'; // optional class
        }
        if ($options['rel']!='') {
            $return .= ' rel="'.$options['rel'].'"';
        }
        ...
        if (!$options['no_label'] && ($label!='' || $options['image']!='')) {
            ...
            if ($options['label_style']!='') {
                $return .= ' style="'.$options['label_style'].'"';
            }
            if ($options['label_extra_attributes']!='') {
                $return .= ' '.$options['label_extra_attributes'];
            }
            $return .= '>';
    ...
    public static function textarea($field_name, $rows=3, $options=array())
    {
        ...
        if ($options['cols']==0 && $options['zero_cols_class']!=='') {
            $classes[] = $options['zero_cols_class'];
        }
        if ($options['with_validation'] && !self::isValid($field_name) && self::$error_class!='') {
            $classes[] = self::$error_class;
        } elseif ($options['with_validation'] && $value!='' && isset($_POST['Submit']) && self::$valid_class!='') {
            $classes[] = self::$valid_class;
        }
        if (count($classes) > 0) {
            $return .= ' class="'.implode(' ',$classes).'"'; // add the classes if there are any
        }
    ...
    public static function quote($what, $type='text', $true_value='', $false_value='')
    {
        ...
        switch ($type) {
            ...
            break;
            default:
                $return = ($return != '') ? "'".trim($return)."'" : 'NULL';
            break;
        }
        self::log(self::pp($return), -1);
        return $return;
    ...
    public static function selectIf($field_name, $compare_value, $type)
    {
        ...
        if (is_int($form_value) && !is_int($compare_value)) {
            $compare_value = intval($compare_value); // convert compare value to integer if the form value is an integer
        }
        if (substr($field_name, -1)==']') {
            self::log('array detected');
            list($field, $index) = explode('[',$field_name); // break on [
            $index = rtrim($index,']'); // remove trailing ]
            if (is_array(self::$form[$field])) {
                $selected = self::$form[$field][$index]==$compare_value;
            } else {
...
////////////////////////////////////////
    ...
    public static function isSpam($field_name)
    {
        ...
        $return = false;
        if (strpos(self::$form[$field_name], 'http://')!==false || strpos(self::$form[$field_name], '</a>'!==false) || strpos(self::$form[$field_name], '[/url]')!==false) {
            $return = true;
        }
        self::log(self::pp($return), -1);
        return $return;
...
////////////////////////////////////////
    ...
    private static function convert($value, $type)
    {
        ...
        switch ($type) {
            ...
            case 'float2':
                ...
                break;
            case 'date':
                $return = !empty($value) ? date(self::$date_format, strtotime($value)) : '';
                break;
            case 'year':
                $return = intval($value>0) ? intval($value) : '';
                break;
            case 'time':
                $return = !empty($value) ? date('g:i a', strtotime($value)) : '';
                break;
            default:
                if (is_array($value)) {
                    ...
                    if (isset($backtrace[2])) {
                        $error .= '; '.$backtrace[2]['file'].' line '.$backtrace[2]['line'];
                    }
    ...
    private static function filterFieldName($field_name, $capitalizer='ucfirst')
    {
        ...
        if (function_exists($capitalizer) && is_callable($capitalizer)) {
            $return = $capitalizer($return);
        }
        self::log(self::pp($return), -1);
        return $return;
    } // filterFieldName()
    ...
    private static function str2Array($string)
    {
        ...
        if (is_array($string)) {
            $return = $string; // already an array, do nothing
        } elseif (strpos($string, ',') !== false) {
            $return = explode(',', $string); // CSV to array
        } elseif ($string!='') {
            $return = array($string); // only one field, so turn it into an array
        } else {
            $return = array_keys(self::$form); // just grab everything from $form
        }
    ...
    public static function pp($expression)
    {
        ...
        if (count($expression)>0) {
            foreach ($expression as $key=>$val) {
                if ($key != '') {
                    $return .= $key.': '.(is_bool($val) ? ($val ? 'true' : 'false') : $val).', ';
                }
            }
        }
